...
const iconStyles = {
  pink: "bg-bloom-pink-light",
  teal: "bg-garden-teal-light",
  gold: "bg-[#fef3dc]",
  midnight: "bg-midnight-bloom/8",
};
...
export default function FeatureCard({
  ...
  return (
    <div className="bg-white rounded-xl p-6 border border-black/6">
      <div
        className={cn(
          "w-11 h-11 rounded-lg flex items-center justify-center text-xl mb-4",
      ...
      {tags.length > 0 && (
        <div className="flex flex-wrap gap-1.5 mt-3">
          {tags.map((tag) => (
            <span
              key={tag}
              className="text-xs px-2.5 py-1 rounded-full border border-black/8 text-text-muted"
            >
              {tag}
            </span>
          ))}
